...
short para_soc[] = {400, 500, 400, 500, 400, 500, 400, 500, 400, 500, 400, 500, 400, 500, 400, 500, 400, 500};
...
static int createFunFrame(int portid, int *pPcsid, int *pLenframe, unsigned char *framebuf)
{
	...
	int curNumPcs;
	static unsigned short xxxx = 0;

	xxxx++;

	if (pParaBams->pcs_num[portid] > pcsid + 7)
	{
		curNumPcs = 7;
	}
	else
		curNumPcs = pParaBams->pcs_num[portid] - pcsid;
	...
	framebuf[pos++] = (numTask * curNumPcs) / 256;
	framebuf[pos++] = (numTask * curNumPcs) % 256;
	len = numTask * 2 * curNumPcs;
	framebuf[pos++] = len;

	for (j = 0; j < curNumPcs; j++)
	{
		for (i = 0; i < numTask; i++)
		{
			if (i == BMS_SOC)
			...
			else if (i == BMS_CONN_HEARTBEAT)
			{
				framebuf[pos++] = xxxx / 256;
				framebuf[pos++] = xxxx % 256;
			}
			else if (i == BMS_single_MX_voltage)
			{
				// if (xxxx > 100 && pcsid == 3) //
				// {
				// 	framebuf[pos++] = 4111 / 256;
				// 	framebuf[pos++] = 4111 % 256;
				// }
				// else
				{
